...
if __name__ == "__main__":
    ...
    try:
        resp = proxy(group_name='scara', names=['0', '1'], families=['Scara', 'Scara'])
    ...
    rospy.wait_for_service('/hebiros/scara/size')
    size_proxy = rospy.ServiceProxy('/hebiros/scara/size', SizeSrv);
    try:
        resp = size_proxy()
        rospy.loginfo("'scara' has size %d" % resp.size);
    except rospy.ServiceException, e:
        rospy.logerr("Service call failed: %s"%e)
