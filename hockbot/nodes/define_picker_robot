...
if __name__ == "__main__":
    ...
    try:
        resp = proxy(
            group_name='picker',
            names=['0', '1', '2', '3', '4'],
            families=['Picker', 'Picker', 'Picker', 'Picker', 'Picker'])
    ...
    rospy.wait_for_service('/hebiros/picker/size')
    size_proxy = rospy.ServiceProxy('/hebiros/picker/size', SizeSrv);
    try:
        resp = size_proxy()
        rospy.loginfo("'picker' has size %d" % resp.size);
    except rospy.ServiceException, e:
        rospy.logerr("Service call failed: %s"%e)
