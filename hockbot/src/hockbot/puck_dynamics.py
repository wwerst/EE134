...
class PuckDynamics(object):
    ...
    def __init__(self):
        ...
        self.TABLE_RIGHT = 0.5 # x Position of the table right edge, meters
        self.TABLE_BOTTOM = 0.0 # y Position of table bottom edge, meters
        self.TABLE_TOP = 2.0 # y Position of table top edge, meters
        self.PUCK_RADIUS = 0.04 # Puck radius, meters
        self.ELASTICITY = 0.6 # Elasticity coefficient, unitless
    ...
    def predict_position(t):
        ...
        # Account for no reverse direction. Assumes vertex is always in future
        ...
        if x < self.TABLE_LEFT + self.PUCK_RADIUS:
            x = self.TABLE_LEFT + ((self.TABLE_LEFT - (x - self.PUCK_RADIUS)) * self.ELASTICITY) + self.PUCK_RADIUS
        elif x > self.TABLE_RIGHT - self.PUCK_RADIUS:
            x = self.TABLE_RIGHT - ((x + self.PUCK_RADIUS - self.TABLE_RIGHT) * self.ELASTICITY) - self.PUCK_RADIUS

        if y < self.TABLE_BOTTOM + self.PUCK_RADIUS:
            y = self.TABLE_BOTTOM + ((self.TABLE_BOTTOM - (y - self.PUCK_RADIUS)) * self.ELASTICITY) + self.PUCK_RADIUS
        elif y > self.TABLE_TOP - self.PUCK_RADIUS:
            y = self.TABLE_TOP - ((y + self.PUCK_RADIUS - self.TABLE_TOP) * self.ELASTICITY) - self.PUCK_RADIUS
